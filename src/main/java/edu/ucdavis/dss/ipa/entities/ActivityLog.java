...
public class ActivityLog {
    ...
    public void setActivityLogTags(List<ActivityLogTag> activityLogTags) {
        this.activityLogTags = activityLogTags;
    }
    ...
    public void addTag(Object referenceEntity) {
        ...
        try {
            Method getEntityId = referenceEntity.getClass()
                .getMethod("getId", (java.lang.Class<?>[])null);
            long entityId = (long) getEntityId.invoke(referenceEntity, (java.lang.Object[])null);

            loggableTag += "_" + entityId;
        } catch (InvocationTargetException e) {
            log.debug("InvocationTargetException when trying method getId on " + loggableTag);
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            log.debug("Illegal Access Exception when trying method getId on " + loggableTag);
            e.printStackTrace();
