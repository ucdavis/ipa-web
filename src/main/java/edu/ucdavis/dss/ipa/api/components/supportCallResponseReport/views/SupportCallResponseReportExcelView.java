...
public class SupportCallResponseReportExcelView extends AbstractXlsxView {
    ...
    private void buildResponsesSheet(Workbook workbook, List<SupportStaff> sortedStaff) {
        ...
        StudentSupportCallResponse sampleResponse = supportCallResponseReportViewDTO.getSupportCallResponses().get(0);

        if (sampleResponse.isCollectAssociateInstructorPreferences() || sampleResponse.isCollectTeachingAssistantPreferences() || sampleResponse.isCollectReaderPreferences()) {
        ...
        for (StudentSupportCallResponse studentResponse : supportCallResponseReportViewDTO.getSupportCallResponses()) {
            List<Object> rowValues = new ArrayList<>(Arrays.asList(studentResponse.getSupportStaff().getLastName(), studentResponse.getSupportStaff().getFirstName()));

            if (studentResponse.isCollectAssociateInstructorPreferences() || studentResponse.isCollectTeachingAssistantPreferences() || studentResponse.isCollectReaderPreferences()) {
                // Set up relation?
                List<StudentSupportPreference> sortedSupportPreferences =
                    supportCallResponseReportViewDTO.getStudentSupportPreferences().stream()
                        .filter(preference -> preference.getSupportStaff().getId() == studentResponse.getSupportStaff().getId())
                ...
                for (int i = 0; i < sortedSupportPreferences.size(); i++) {
                    ...
                    cellString += preference.getPriority() + ") "
                        ...
                        + (preference.getType().equals("Teaching Assistant") ? "Teaching Assistant" : "Reader")
            ...
            if (studentResponse.isCollectLanguageProficiencies()) {
                // add enum for string values
                rowValues.add(studentResponse.getLanguageProficiency());
            }
